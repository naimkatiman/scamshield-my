// ──────────────────────────────────────────────────────
// Realistic mock data for Malaysian scam intelligence
// ──────────────────────────────────────────────────────
...
const feedTemplates: Record<LiveFeedItem['type'], string[]> = {
  detection: [
    ...
    'AI flagged suspicious {category} activity on {platform}',
  ],
  report: [
    ...
    'Multiple reports received: {platform} {category} campaign',
  ],
  recovery: [
    ...
    'Recovery checklist completed — {category} case',
  ],
  alert: [
    ...
    'HIGH PRIORITY: {category} syndicate active on {platform}',
  ],
}
...
function generateFeedItem(index: number): LiveFeedItem {
  ...
  return {
    ...
    severity,
    message,
    value: type === 'recovery' ? rmValue : undefined,
  }
}
...
export function generateHeatmap(): HeatmapCell[] {
  ...
  for (const platform of heatmapPlatforms) {
    for (const category of heatmapCategories) {
      ...
      const trends: HeatmapCell['trend'][] = ['up', 'down', 'stable']
      const trendWeights = intensity > 0.6 ? [0.6, 0.15, 0.25] : [0.2, 0.4, 0.4]
      const r = Math.random()
      const trend = r < trendWeights[0] ? 'up' : r < trendWeights[0] + trendWeights[1] ? 'down' : 'stable'
...
export const threatCampaigns: ThreatCampaign[] = [
  {
    ...
    status: 'active',
    description: 'Impersonating LHDN officers with spoofed caller ID. Demands immediate payment via crypto transfer.',
  },
  {
    id: 'tc-002',
    name: 'GOLDMINE INVEST',
    platform: 'Telegram',
    category: 'Investment',
    severity: 'critical',
    victims: 891,
    ...
    status: 'active',
    description: 'Fake gold trading platform promising 15% weekly returns. Uses celebrity deepfakes for promotion.',
  },
  {
    id: 'tc-003',
    name: 'SHOPEE PHANTOM',
    ...
    status: 'active',
    description: 'Clone stores selling electronics at 70% discount. Products never delivered. Uses stolen merchant identities.',
  },
  {
    id: 'tc-004',
    name: 'PART-TIME KING',
    ...
    status: 'monitoring',
    description: 'Fake part-time job ads requiring upfront "training deposit". Targets students and gig workers.',
  },
  {
    id: 'tc-005',
    name: 'LOVE TRAP MY',
    ...
    status: 'monitoring',
    description: 'Romance scam network using AI-generated profiles. Targets divorced individuals aged 40-60.',
  },
]
